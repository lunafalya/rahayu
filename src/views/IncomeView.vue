      ...
      <!-- Income -->
        <div class="ml-30 w-full p-8 px-6 pt-12 flex gap-6 ">
          ...
          <div class="bg-white rounded-2xl p-6 shadow-lg h-full w-full">
            <h2 class="text-3xl font-bold mb-4 text-cyan-950" style="font-family: 'Poppins', sans-serif;">REKENING</h2>
            <div class="mb-6">
              <p class="text-sm text-gray-300" style="font-family: 'Poppins', sans-serif;">Saldo Tersedia</p>
            ...
            <div class="flex gap-4 mb-6">
              <div class="flex items-center gap-2 bg-white rounded-xl px-4 py-2 shadow">
                ...
                <div>
                  <p class="text-xs text-gray-400" style="font-family: 'Poppins', sans-serif;">Pengeluaran</p>
                  <p class="text-sm font-semibold text-cyan-950" style="font-family: 'Poppins', sans-serif;"></p>
                </div>
              </div>
              <div class="flex items-center gap-2 bg-white rounded-xl px-4 py-2 shadow">
                <div class="bg-blue-400 p-2 rounded-full">
                  <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24" style="font-family: 'Poppins', sans-serif;">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14m0 0l-6-6m6 6l6-6" />
                  </svg>
                </div>
                <div>
                  <p class="text-xs text-gray-400" style="font-family: 'Poppins', sans-serif;">Pemasukan</p>
          ...
          <div v-if="transactions.length != 0" class="overflow-y-auto max-h-[40vh]">
            <ul v-for="(transaction, index) in transactions.data" :key="index" class="text-sm m-2">
              <li class="flex justify-between items-center">
                <div>
                  <p style="font-family: 'Poppins', sans-serif;" class="font-semibold text-cyan-950">{{ transaction.channel_category }} - <span v-if="transaction.status === 'SUCCESS'" class="text-green-400">{{ transaction.status }}</span> <span v-else>{{ transaction.status }}</span></p>
                  <p style="font-family: 'Poppins', sans-serif;" class="text-gray-400 text-xs">{{ Intl.DateTimeFormat('id-ID', {year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit'}).format(new Date(transaction.updated)) }}</p>
                </div>
          ...
          <div v-else class="flex justify-center items-center h-full">
            <p class="text-gray-400" style="font-family: 'Poppins', sans-serif;">Tidak ada transaksi</p>
          </div>
        </div>
      </div>
      </div>
